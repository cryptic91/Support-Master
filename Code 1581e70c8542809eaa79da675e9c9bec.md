...
Rebuy drawer cart loads the dom later, for which our common.js file doesn't work on the drawer cart because it loads too fast. The code below can be used before the </head> closing tag, and the common.js script will load later. It will display the personalized image and details in our format. This is for the Rebuy Drawer cart only. 
